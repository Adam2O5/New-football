...
void main() {
  ...
  test('simulateUntilDate stops exactly at the target date', () async {
    final result = await controller.simulateUntilDate(1, 2);
  ...
  test(
    'simulateUntilPhaseEnd(regular) reaches the start of play-in when there '
    'is no player team to pause on',
    () async {
  ...
  test('cancelSimulation stops the batch after the current day', () async {
    ...
    // first `await simulateDay()` — cancelling now is picked up on the next
    // loop check, deterministically stopping after exactly one day.
  ...
  test('draft pick pending stops the batch for the player\'s turn', () async {
    ...
    final result = await controller.simulateUntilDate(48, 1);
    expect(result.stopReason, SimulationStopReason.draftPick);
